...
public class StudentServiceImpl implements StudentService {
    ...
    /**
     * ✅ NEW: Implements the QR Scanning Logic.
     * Marks a student as present for a specific course today.
     */
    @Override
    @Transactional
    public String markAttendanceViaQr(Long courseId, Long studentId) {
        // 1. Check for existing attendance to prevent duplicate marking
        boolean alreadyMarked = attendanceRepository.existsByStudentIdAndCourseIdAndDate(
                studentId,
                courseId,
                LocalDate.now());
        ...
        // 2. Fetch required entities from database
        Student student = studentRepository.findById(studentId)
                .orElseThrow(() -> new RuntimeException("Student not found"));
        Course course = courseRepository.findById(courseId)
                .orElseThrow(() -> new RuntimeException("Course not found"));

        // 3. Create and persist the new Attendance record
        Attendance attendance = new Attendance();
        attendance.setStudent(student);
        attendance.setCourse(course);
        attendance.setDate(LocalDate.now());
        attendance.setStatus("PRESENT"); // Ensure this matches your Entity's Status field type

        attendanceRepository.save(attendance);

        return "Attendance successfully marked for: " + course.getCourseName();
    }
    ...
    @Override
    public List<AttendanceDetailResponse> getStudentAttendance(Long studentId, Long courseId, LocalDate startDate,
            LocalDate endDate) {
        Student student = studentRepository.findById(studentId)
                .orElseThrow(() -> new RuntimeException("Student not found"));
